...
**Цикл фазы (9 шагов):** Scope → Plan(pinned) → Domain-build(профильный агент пишет сам) → Implement → Review(≤2 цикла) → Verify → **Post-audit** → **Memory-update** → PR + аппрув founder.
...
**Founder:** открой [`STATUS.md`](.planning/STATUS.md) → выбери фазу → запусти 9-шаговый цикл (главная сессия = оркестратор, делегирует профильным агентам). Аппрувишь PR на tier 3+.
