...
| 6 | **Длительность trial vs наблюдаемость Visibility.** Финализировать (14 vs 21 vs 30 дней) по данным Phase 0 (H4 — time-to-citation) и первых design-партнёров. Readiness — мгновенный хук; Visibility — нужно ≥21–30 дн. **Дефолт зафиксирован (ADR-0023): 14 дн self-serve / 30 дн партнёры**, `revisit_after` P0/H4 + первые данные. | founder + geo-domain-expert | P0/H4 → P9 (billing design) | in-progress |
| 7 | **Порог auto-apply (trust ladder).** Какие именно типы машиночитаемых правок дефолтно разрешать в авто-режим после скольких успешных approve-циклов. **Дефолт зафиксирован (ADR-0023): approval-gate на всё + per-type opt-in доступен сразу (без порога)**; FAQ никогда не auto. `revisit_after` P10 прод-данные (rollback-rate по типам). | geo-domain-expert + cms-connector-specialist + founder | P10 (design phase) | in-progress |
| 8 | **Конверсия воронки Tier 0 → Manual → Auto.** Целевые коэффициенты конверсии, где основной upsell в Auto track, оптимальная разница цен Manual vs Auto. **Дефолт зафиксирован (ADR-0023): цена Manual = Auto** (гейт Auto = DPA, не деньги — усиливает конверсию в moat/data-flywheel); коэффициенты — `revisit_after` P7+P9 прод-данные. | founder | P7+P9 → A→B gate | in-progress |
