...
## 2026-07-03 · oriion-methodology-integration · @claude-opus (ADR-0020 + ADR-0021)

- **Scope:** запрос founder — «примени автономную методологию ORIION, интегрируй полноценно». Интервью (AskUserQuestion ×3): гэп = **исполняемый слой + реальная автономность с мин. включением**; источник = **добавить ORIION-репо** (`mrflxxxme/oriion`, добавлен через add_repo + clone); политика = **сохранять lean** (ADR-0001 в силе, отмена ADR только через новый ADR).
- **Диагноз:** методология WIZOR была описана ПРОЗОЙ, но не ИСПОЛНЯЛАСЬ (ноль `.claude/commands/`, хуков, settings, autonomy-рантайма). ORIION имеет ровно этот слой — **ADR-037 автономный runner** (8 решений D1–D8, Блоки A–E). WIZOR уже имел концепты (ADR-0017 gate-only, ADR-0018 live-gold, §6 инварианты), но не enforcement-машину.
- **Решение:** **ADR-0020** (исполняемый слой) + **ADR-0021** (порт ADR-037). Построено: `.claude/autonomy/` (tripwire 8 кат. · evidence-schema · escalation-policy · judge-panel · README+BUILD-PLAN · founder-armed settings/hook/notify snippets); `scripts/autonomy/` (8 скриптов, порт через 2 параллельных суб-агента по shared-спеке); `.claude/commands/autonomy/` (run/discuss/ack/heal); `.github/workflows/evidence.yml`; seeds RUN-QUEUE + DECISIONS-LOG + evidence/README.
- **Lean-адаптации (Q3):** role-loader (`load_role.py`) вместо flat-суб-агентов (ноль дублей, ORIION-паттерн) → **ADR-0020 пересмотрен** по ходу; `auditor` вместо ORIION-`evaluator` как судья judge-панели; PNN-фазы вместо decimal; §6-инварианты → 3 WIZOR-tripwire-категории (autofix_writes/probe_geo/pdn_compliance) сверх 5 базовых ORIION.
- **Verify:** py_compile ×8 зелёные (суб-агенты); smoke в WIZOR-env — `load_role --list` = 14 ролей, `verify_evidence` exit 0 (no manifest), `classify_tripwire` чистый JSON, `run_queue pending` OK, `log_decision` создал DECISIONS-LOG (dogfood: записана сама интеграция). PyYAML 6.0.1 в env есть.
- **Safety:** машина установлена **ВЫКЛЮЧЕННОЙ** (founder-armed, как ORIION). Auto-mode classifier дважды заблокировал self-install settings.json/хука → корректно оформлено как founder-armed snippets (`.claude/autonomy/README §Вооружение`).
