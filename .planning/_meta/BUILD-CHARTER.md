...
9. **PR + авто-мердж** — PR открывается **сразу открытым (ready-for-review), НЕ draft** (ADR-0019) → CI-чеки и AI-ревью стартуют немедленно, фейлы чинятся автономно в цикле; внутри фазы PR мерджится автономно (зелёный CI + `reviewer` APPROVE + `auditor` PASS + **verify green: тесты + live-gold/deferral**, ADR-0018); **человек-аппрув ТОЛЬКО на гейте фазы** (`founder_signature`); squash-merge (ADR-0017).
