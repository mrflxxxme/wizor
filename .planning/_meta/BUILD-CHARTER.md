...
6. **Verify** (`verifier`/Haiku) — acceptance + **авто-тесты (unit+integration) + live-gold где возможно** (ADR-0018) + пороги гейта, бинарно; evidence в `_session-context/VERIFY-*`; невозможность live-gold → явный deferral в гейт.
...
9. **PR + авто-мердж** — внутри фазы PR мерджится автономно (зелёный CI + `reviewer` APPROVE + `auditor` PASS + **verify green: тесты + live-gold/deferral**, ADR-0018); **человек-аппрув ТОЛЬКО на гейте фазы** (`founder_signature`); squash-merge (ADR-0017).
...
**Автономность подтверждается результатами.** Перед PR `verifier` (шаг 6) обязан получить зелёный:
- **Авто-тесты:** unit + integration (coverage ≥70% / ≥85% security-critical).
- **Live-gold, где возможно:** end-to-end против РЕАЛЬНЫХ сервисов с малым golden-набором (funded-ключи/тест-сайт/прокси). По треку: read-only → живой crawl/probe golden; infra → живой smoke (PG/Redis/Keycloak); auto (P10) → применение на founder-owned тест-WP + живой rollback.
- **Evidence:** `_session-context/VERIFY-<phase>-<ts>.md` (секции `## Tests`, `## Live-gold`); гейт ссылается через `evidence_url`.
...
*Charter v1.2 · 2026-06-23 · источник истины для всех build-агентов WIZOR. Изменения — только через ADR. v1.1: +ADR-0017 (автономия; human-чекпоинт только на гейтах). v1.2: +ADR-0018 (тесты+live-gold перед PR; подтверждённая результатами автономность).*
